...
Api.prototype = {
    ...
    _servicesLoader: function() {
        ...
        var servicesRoot = require.context("./services", true, /\.js$/);
        servicesRoot.keys().forEach(function (key) {
            var serviceName = key.match(/(.*)\.js/)[1].substring(2);
            var ServiceClass = servicesRoot(key)
        ...
        self.models = {};
        var modelsRoot = require.context("./models", true, /\.js$/);
        modelsRoot.keys().forEach(function (key) {
            var modelName = key.match(/(.*)\.js/)[1].substring(2);
            self.models[modelName] = modelsRoot(key)
        });
    }
};
